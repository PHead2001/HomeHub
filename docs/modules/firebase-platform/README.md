...
No explicit Firestore offline persistence, Auth persistence override, App Check, emulator connection, or custom local-cache policy is configured.
...
Update this README when Firebase initialization, environment names, deployment config, collection/Storage topology, security rules, indexes, Functions, Auth/membership fallback, App Check/emulator/offline settings, or TTL requirements change.
